<?php
require_once 'system/classes/class-entityAdditionalAttribute.php';
...
class Entity {
	...
	public function setName($name) {
		...
		$stmt = $conn->prepare('UPDATE entity SET name = :name WHERE id = :id');
	...
	public function updateEntityNameAndDescription($name, $description) {
		$db = Database::getInstance();
		$conn = $db->connect();
	
		$stmt = $conn->prepare('UPDATE entity SET name = :name, description = :description WHERE id = :id');
	
		if ($stmt->execute(array('name' => $name, 'description' => $description, 'id' => $this->id))) {
			if ($stmt->rowCount() >= 1) {
				$this->name = $name;
				$this->description = $description;
				return true;
...
class EntityAPI {
	...
	static public function updateEntityNameAndDescription($id, $name, $description) {
		$db = Database::getInstance();
		$conn = $db->connect();
		
		$stmt = $conn->prepare('UPDATE entity SET name = :name, description = :description WHERE id = :id');
		
		if ($stmt->execute(array('name' => $name, 'description' => $description, 'id' => $id))) {
			if ($stmt->rowCount() >= 1) 
				return true;
			else
				return false;
